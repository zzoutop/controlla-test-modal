...
app = modal.App("controlla-test-modal")


@app.cls(gpu="l40s", image=image, volumes={trained_cache_dir: trained_cache})
...
@app.function(
    ...
    max_containers=1,
    volumes={trained_cache_dir: trained_cache},
)
...
def ui():
    ...
    existing_checkpoints = [
        checkpoint.name
        for checkpoint in sub_folders
        if checkpoint.name.endswith("lora")
    ]
    lora_choices = ["none"] + existing_checkpoints
    ...
    with gr.Blocks(theme="soft") as demo:
        ...
        with gr.Row():
            with gr.Column():
                lora = gr.Dropdown(label="Lora", choices=lora_choices)
        ...
        btn.click(
            generate_music,
            inputs=[prompt, lyrics, duration, format, lora],
            outputs=[clip_output],
        )
